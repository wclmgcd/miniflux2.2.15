...
func NewConfigOptions() *configOptions {
	return &configOptions{
		...
		options: map[string]*configValue{
			...
			"CACHE_LOCATION": {
				ParsedStringValue: "disk",
				RawValue:          "disk",
				ValueType:         stringType,
				Validator: func(rawValue string) error {
					return validateChoices(rawValue, []string{"disk", "database"})
				},
			},
			"CACHE_INTERVAL": {
				ParsedDuration: 24 * time.Hour,
			...
			"DISK_STORAGE_ROOT": {
				...
				RawValue:          "./",
				ValueType:         stringType,
				// Validator:         func(rawValue string) error { return nil },
			},
		},
	}
}
...
func (c *configOptions) CacheInterval() time.Duration {
	return c.options["CACHE_INTERVAL"].ParsedDuration
}

func (c *configOptions) DiskStorageRoot() string {
	return c.options["DISK_STORAGE_ROOT"].ParsedStringValue
}

func (c *configOptions) CacheLocation() string {
	return c.options["CACHE_LOCATION"].ParsedStringValue
}
